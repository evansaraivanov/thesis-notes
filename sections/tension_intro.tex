...
\begin{equation}
	\delta\theta = \sqrt{\mathrm{var}(\theta_{\mathrm{DES}}) + \mathrm{var}(\theta_{\mathrm{Planck}})}\,.
\end{equation}
They then compute the tension metrics and compare to the a-priori gaussian tension. The main result demonstrates what the resulting tension is from a fixed shift along a single parameter. Their result is presented in this thesis in figure~\ref{figure}. An additional note is that gaussian tension tends to underestimate the tension presented in other metrics, highlighting the importance of capturing non-gaussianities.
